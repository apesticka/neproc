...
bfs([[State|Prev]|_], [State|Prev], Initial, _) :- finalState(Initial, State).
bfs([[State|Prev]|Queue], Result, Initial, Len) :-
    findall([NewState, State|Prev], action(State, Len, NewState), NewPaths),
    append(Queue, NewPaths, NewQueue),
    bfs(NewQueue, Result, Initial, Len), !.

reconstructPath(Initial, Positions, Path) :-
    maplist(flipAll(Initial), PathR, Positions),
    reverse(PathR, Path).
